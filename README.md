...
\qquad
\mathbf r_t = \bigl(r_t^{(1)},\dots,r_t^{(N)}\bigr)^\top \in \mathbb R^N.
$$

The pipeline is:

1. compress \(\mathbf r_t\) using adaptive PCA,
2. map latent scores into spins \(s_j\in\{-1,0,+1\}\),
...
$$
\mathcal W_t=\{\mathbf r_{t-W+1},\mathbf r_{t-W+2},\dots,\mathbf r_t\},
$$

and form the centered matrix

$$
\widetilde{\mathbf R}_t=\mathbf R_t-\mathbf 1_W\boldsymbol\mu_t^\top,
\qquad
\boldsymbol\mu_t=\frac{1}{W}\mathbf R_t^\top \mathbf 1_W.
$$

Using the singular value decomposition

$$
\widetilde{\mathbf R}_t=\mathbf U\boldsymbol\Sigma \mathbf V^\top,
$$

the loading matrix is

$$
\mathbf P_t=\mathbf V_{:,1:n}\in\mathbb R^{N\times n},
$$

and the latent score vector for a return observation \(\mathbf r\) is

$$
\mathbf z=\mathbf P_t^\top(\mathbf r-\boldsymbol\mu_t)\in\mathbb R^n.
$$

## 3. Three-state spin encoding

Each principal component score \(z_j\) is discretised into a spin variable

$$
s_j\in\{-1,0,+1\}.
$$

A corrected z-score thresholding rule is used. Let \(\Phi\) be the standard normal CDF and let

$$
z^\star=\Phi^{-1}(1-q), \qquad q\in\left(0,\frac13\right].
$$

With training standard deviation \(\sigma_j=\operatorname{std}(\{z_{t,j}\})\), the thresholds are
...
s_j=
\begin{cases}
-1, & z_j<\tau_j^{\mathrm{lo}},\\[4pt]
0, & \tau_j^{\mathrm{lo}}\le z_j\le \tau_j^{\mathrm{hi}},\\[4pt]
...
For a spin configuration \(\mathbf x\in\{-1,0,+1\}^n\), the Blume–Capel Hamiltonian is
...
-\sum_{1\le i<j\le n} J_{ij}x_i x_j,
$$

where \(\theta=(\mathbf h,\mathbf d,\mathbf J)\), \(\mathbf J=\mathbf J^\top\), and \(J_{ii}=0\).

The associated distribution is

$$
p(\mathbf x;\theta)=\frac{1}{Z(\theta)}\exp\!\bigl(-H(\mathbf x;\theta)\bigr),
\qquad
Z(\theta)=\sum_{\mathbf x\in\{-1,0,+1\}^n}\exp\!\bigl(-H(\mathbf x;\theta)\bigr).
$$

### 4.1 Mean-field approximation

Because exact inference is exponential in \(n\), RARSI uses a factorised approximation

$$
q(\mathbf x)=\prod_{i=1}^n q_i(x_i).
$$

The effective field at node \(i\) is
...
The one-site marginals satisfy

$$
q_i(+1)\propto e^{d_i+h_i^{\mathrm{eff}}},
\qquad
q_i(0)\propto 1,
\qquad
q_i(-1)\propto e^{d_i-h_i^{\mathrm{eff}}}.
...
m_i=q_i(+1)-q_i(-1),
\qquad
q_i^{(2)}=\mathbb E_q[x_i^2]=q_i(+1)+q_i(-1)=1-q_i(0).
$$

The fixed point is obtained by iterating until
...
$$
\mathrm{PLL}(\mathbf x;\theta)
=
\sum_{i=1}^n \log p(x_i\mid \mathbf x_{-i};\theta).
$$

The useful gradients are

$$
\frac{\partial\,\mathrm{PLL}}{\partial h_i}=x_i-m_i,
\qquad
\frac{\partial\,\mathrm{PLL}}{\partial d_i}=x_i^2-q_i^{(2)},
\qquad
\frac{\partial\,\mathrm{PLL}}{\partial J_{ij}}=x_i x_j-m_i m_j
\quad (i\ne j).
$$
...
$$
p(\mathbf x_t\mid C_t=k)=p_{\mathrm{BC}}(\mathbf x_t;\theta^{(k)}).
$$

### 5.1 Sticky transition prior

A simple sticky prior is
...
$$
\alpha_{t,k}=P(C_t=k\mid \mathbf x_{1:t}).
$$

To avoid underflow, the forward update is performed in log-space:

$$
\log \widetilde{\alpha}_{t,k}
=
\log\!\left(\sum_{i=1}^K \exp(\log A_{ik}+\log \alpha_{t-1,i})\right)
+
\frac{\log e_{t,k}}{\tau},
$$

where \(e_{t,k}=p(\mathbf x_t\mid C_t=k)\) and \(\tau\ge 1\) is an emission temperature. Normalisation is then
...
\log \widetilde{\alpha}_{t,k}
-
\operatorname{logsumexp}(\log \widetilde{\boldsymbol\alpha}_t).
$$

The \(H\)-step regime forecast is

$$
\boldsymbol\alpha_{t+H}=A^H\boldsymbol\alpha_t.
$$
...
L_t^{(k)}(\theta^{(k)})
=
w_{t,k}\,\mathrm{PLL}(\mathbf x_t;\theta^{(k)})
-
\Omega(\theta^{(k)}),
\qquad
w_{t,k}=\alpha_{t,k}.
...
\Omega(\theta)
=
\frac{\lambda_2}{2}\lVert \mathbf h\rVert_2^2
+
\frac{\lambda_2^{(d)}}{2}\lVert \mathbf d\rVert_2^2
+
\frac{\lambda_2}{2}\lVert \mathbf J\rVert_F^2
+
\lambda_1^{(J)}\lVert \mathbf J\rVert_{1,\mathrm{off\text{-}diag}}.
$$

AdaGrad-style preconditioning uses

$$
G_t=G_{t-1}+\operatorname{diag}(g_t\odot g_t),
\qquad
\theta_{t+1}
=
\theta_t
-
\eta\,(G_t^{1/2}+\varepsilon I)^{-1}g_t.
$$
...
\xi_{ij}
=
\frac{\alpha_{t-1,i}A_{ij}e_{t,j}}{\sum_{k=1}^K \alpha_{t-1,i}A_{ik}e_{t,k}}.
$$
...
\bar{\mathbf s}_{t+H}
=
\sum_{k=1}^K \alpha_{t+H,k}\,\mathbf m^{(k)}.
$$

The Blume–Capel signal in asset space is then

$$
\mathbf s_t^{\mathrm{BC}}=\mathbf P_t\,\bar{\mathbf s}_{t+H}.
$$
...
standardised cross-sectionally and averaged across windows.

The final blend uses rolling Information Coefficients, i.e. Spearman rank correlations,

$$
\rho_S(\mathbf s,\mathbf r).
$$
...
\qquad
w_i \leftarrow \operatorname{clip}(w_i,-w_{\max},w_{\max}),
$$

followed by re-normalisation. Volatility targeting uses

$$
L_t=\operatorname{clip}\!\left(\frac{\sigma_{\mathrm{target}}}{\sigma_{\mathrm{ann}}+10^{-6}},\,0.1,\,L_{\max}\right).
$$

Turnover and transaction costs are

$$
\mathrm{TO}_t=\lVert \mathbf w_t-\mathbf w_{t-1}\rVert_1,
\qquad
...
At each out-of-sample step \(t\in\{W,\dots,T-H\}\):

1. fit PCA on \(\mathcal W_t\),
2. encode latent scores into spins,
3. fit the regime-specific Blume–Capel models,
4. infer \(\boldsymbol\alpha_t\) with the HMM filter,
...
\mathrm{Sharpe}
=
\frac{\bar r-r_f}{\operatorname{std}(\{r_t-r_f\})}\sqrt{252},
\qquad
r_f=\frac{0.065}{252}.
$$
...
$$
\mathrm{MDD}=\min_t \frac{R_t-\max_{\tau\le t}R_\tau}{\max_{\tau\le t}R_\tau},
\qquad
\mathrm{Calmar}=\frac{\mu_{\mathrm{ann}}}{|\mathrm{MDD}|}.
$$
